...
class Adapter() : RecyclerView.Adapter<Adapter.ItemViewHolder>() {
    ...
    override fun getItemCount() = ClosetData.values().size
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        //데이터 바인딩
        holder.bindData(ClosetData.values()[position])
    ...
    inner class ItemViewHolder(private val binding: ListItemBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bindData(ClosetData: ClosetData) {
            binding.tvCloset.text = ClosetData.closetName
            binding.tvSerial.text = ClosetData.Serial
            binding.imgPhoto.setImageResource(ClosetData.imgRes)
